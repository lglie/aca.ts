      ...
      switch (v.kind) {
        ...
        case ts.SyntaxKind.ModuleDeclaration: // module declaration
          ...
          if (_[name]) {
            _[name].models = {
              ..._[name].models,
              ...Iter(<ts.ModuleBlock>moduleV.body, [...namespace, name])
            }
          } else {
            _[name] = {
              kind: 'namespace',
        ...
        case ts.SyntaxKind.ClassDeclaration: // is table declaration
          ...
          body['columns'] = classV.members.reduce((_2, v2) => {
            ...
            props = NodeParse.decorator(
              ts.canHaveDecorators ? ts.getDecorators(V2) : V2.decorators || []
            )
            MR = MapRename(props)
...
export default async function (acaDir: '.' | '..', content: string = '') {
  ...
  if (!content) {
    ...
    for (const key of config.orm) {
      const ormPath = path.join(acaRoot, Cst.AcaDir, key)
      if (fs.existsSync(ormPath)) {
        Iter(ormPath)
      } else {
        throw new Error('No found schema in path: ' + ormPath)
      }
    }
